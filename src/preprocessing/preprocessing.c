...
void preprocess_tbox(KB* kb) {
	...
	while (ax) {
		ADD_GENERATED_SUBCLASS_OF_AXIOM(create_subclass_axiom(((EquivalentClassesAxiom*) ax)->lhs, ((EquivalentClassesAxiom*) ax)->rhs), tbox);
		ADD_GENERATED_SUBCLASS_OF_AXIOM(create_subclass_axiom(((EquivalentClassesAxiom*) ax)->rhs, ((EquivalentClassesAxiom*) ax)->lhs), tbox);
	...
	ax = SET_ITERATOR_NEXT_64(&set_iterator);
	while (ax) {
		ADD_GENERATED_SUBOBJECTPROPERTY_OF_AXIOM(create_subrole_axiom(((EquivalentObjectPropertiesAxiom*) ax)->lhs, ((EquivalentObjectPropertiesAxiom*) ax)->rhs), tbox);
		ADD_GENERATED_SUBOBJECTPROPERTY_OF_AXIOM(create_subrole_axiom(((EquivalentObjectPropertiesAxiom*) ax)->rhs, ((EquivalentObjectPropertiesAxiom*) ax)->lhs), tbox);
		ax = SET_ITERATOR_NEXT_64(&set_iterator);
	}

	// Process the transitive role axioms
	ObjectPropertyExpressionId composition;
	...
	while (ax) {
		...
		ADD_GENERATED_SUBOBJECTPROPERTY_OF_AXIOM(create_subrole_axiom(composition, ((TransitiveObjectPropertyAxiom*) ax)->r), tbox);
		ax = SET_ITERATOR_NEXT_64(&set_iterator);
	}
		...
		for (i = 0; i < ((DisjointClassesAxiom*) ax)->classes.size - 1; ++i)
			for (j = i + 1; j < ((DisjointClassesAxiom*) ax)->classes.size ; ++j) {
				...
				conjunction = get_create_conjunction_binary(
						((DisjointClassesAxiom*) ax)->classes.elements[i],
						((DisjointClassesAxiom*) ax)->classes.elements[j], tbox);
	...
	ax = SET_ITERATOR_NEXT_64(&set_iterator);
	while (ax) {
		ClassExpressionId lhs = get_create_exists_restriction(((ObjectPropertyDomainAxiom*) ax)->object_property_expression, tbox->top_concept, tbox);
		ADD_GENERATED_SUBCLASS_OF_AXIOM(create_subclass_axiom(lhs, ((ObjectPropertyDomainAxiom*) ax)->class_expression), tbox);
		ax = SET_ITERATOR_NEXT_64(&set_iterator);
	}

	// Process the SameIndividual axioms
	SET_ITERATOR_INIT_64(&set_iterator, &(tbox->same_individual_axioms));
	SameIndividualAxiom* same_individual_ax = SET_ITERATOR_NEXT_64(&set_iterator);
	while (same_individual_ax) {
		int i;
		for (i = 0; i < same_individual_ax->individuals.size - 1; ++i) {
			ADD_GENERATED_SUBCLASS_OF_AXIOM(create_subclass_axiom(
					get_create_generated_nominal(tbox, same_individual_ax->individuals.elements[i]),
					get_create_generated_nominal(tbox, same_individual_ax->individuals.elements[i+1])), tbox);
			ADD_GENERATED_SUBCLASS_OF_AXIOM(create_subclass_axiom(
					get_create_generated_nominal(tbox, same_individual_ax->individuals.elements[i+1]),
					get_create_generated_nominal(tbox, same_individual_ax->individuals.elements[i])), tbox);
		}
		same_individual_ax = SET_ITERATOR_NEXT_64(&set_iterator);
	}
	...
	DifferentIndividualsAxiom* different_individuals_ax = SET_ITERATOR_NEXT_64(&set_iterator);
...
void preprocess_abox(KB* kb) {
	...
	for (i = 0; i < abox->concept_assertion_count; ++i)
		ADD_GENERATED_SUBCLASS_OF_AXIOM(
				create_subclass_axiom(
						get_create_generated_nominal(kb->tbox, abox->concept_assertions[i]->individual),
						abox->concept_assertions[i]->concept),
				kb->tbox);
	...
	for (i = 0; i < abox->role_assertion_count; ++i)
		ADD_GENERATED_SUBCLASS_OF_AXIOM(
			create_subclass_axiom(
					get_create_generated_nominal(kb->tbox, abox->role_assertions[i]->source_individual),
					get_create_generated_exists_restriction(kb->tbox, abox->role_assertions[i]->role,
							get_create_generated_nominal(kb->tbox, abox->role_assertions[i]->target_individual)
					)
				),
			kb->tbox
		);
