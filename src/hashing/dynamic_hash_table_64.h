...
#ifndef DYNAMIC_HASH_TABLE_64_H_
#define DYNAMIC_HASH_TABLE_64_H_
...
typedef struct dynamic_hash_table_64 DynamicHashTable_64;
typedef struct dynamic_hash_table_iterator_64 DynamicHashTableIterator_64;

#define HASH_TABLE_EMPTY_KEY_64		NULL
#define HASH_TABLE_DELETED_KEY_64		((void*) -1)

/**
 * Hash table.
 */
struct dynamic_hash_table_64 {
 ...
 */
struct dynamic_hash_table_iterator_64 {
	DynamicHashTable_64* hash_table;
 ...
 */
inline char dynamic_hash_table_insert_64(void* key, DynamicHashTable_64* hash_table) {
	int i, j, new_size;
	size_t start_index;

	assert(key != HASH_TABLE_EMPTY_KEY_64 && key != HASH_TABLE_DELETED_KEY_64);
	...
	for (i = start_index; ; i = (i + 1) & (hash_table->size - 1)) {
		...
		if (hash_table->elements[i] == HASH_TABLE_EMPTY_KEY_64) {
		...
		for (i = 0; i < hash_table->size; ++i)
			if (hash_table->elements[i] != NULL) {
				...
				for (j = start_index; ; j = (j + 1) & (new_size - 1))
					if (tmp_elements[j] == HASH_TABLE_EMPTY_KEY_64) {
 ...
 */
inline char dynamic_hash_table_remove_64(void* key, DynamicHashTable_64* hash_table) {
	assert(key != NULL);
	...
	for (i = start_index; i != hash_table->end_indexes[start_index]; i = (i + 1) & (hash_table->size - 1)) {
		if (hash_table->elements[i] == key) {
			...
			hash_table->elements[i] = HASH_TABLE_DELETED_KEY_64;
			return 1;
		}
	}
	return 0;
 ...
 */
inline char dynamic_hash_table_contains_64(void* key, DynamicHashTable_64* hash_table) {
	// assert(key != NULL);
	size_t start_index = HASH_POINTER(key) & (hash_table->size - 1);
...
inline DynamicHashTableIterator_64* dynamic_hash_table_iterator_create_64(DynamicHashTable_64* hash_table) {
	DynamicHashTableIterator_64* iterator = (DynamicHashTableIterator_64*) malloc(sizeof(DynamicHashTableIterator_64));
...
inline void dynamic_hash_table_iterator_init_64(DynamicHashTableIterator_64* iterator, DynamicHashTable_64* hash_table) {
...
inline void* dynamic_hash_table_iterator_next_64(DynamicHashTableIterator_64* iterator) {

	int i;
	for (i = iterator->current_index; i < iterator->hash_table->size; ++i)
		if (iterator->hash_table->elements[i] != HASH_TABLE_EMPTY_KEY_64 && iterator->hash_table->elements[i] != HASH_TABLE_DELETED_KEY_64) {
 ...
 */
int dynamic_hash_table_iterator_free_64(DynamicHashTableIterator_64* iterator);
