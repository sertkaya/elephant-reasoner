...
typedef DynamicHashMap Map;
typedef DynamicHashMapIterator MapIterator;
 ...
 */
#define MAP_CREATE(size)				dynamic_hash_map_create(size)

/**
 * Initialize a map with an underlying hash map of the given size.
 */
#define MAP_INIT(map, size)				dynamic_hash_map_init(map, size)

/**
 * Adds the pair (key,value) to the map m.
 * Returns 1 if e is successfully added, 0 otherwise.
 */
#define MAP_PUT(key, value, map)		dynamic_hash_map_put(map, key, value)
 ...
 */
#define MAP_REMOVE(key, map)			dynamic_hash_map_remove(key, map)

/**
 * Get the value associated with the given key.
 * Returns 1 if the key occurs, 0 otherwise
 */
#define MAP_GET(key, map)				dynamic_hash_map_get(map, key)

/**
 * Free the space allocated for the given map.
 * Returns the number of freed bytes.
 */
#define MAP_FREE(map)					dynamic_hash_map_free(map)

/**
 * Create an iterator for the elements of the map.
 * It is the iterator of the underlying hash map.
 */
#define MAP_ITERATOR_CREATE(map)		dynamic_hash_map_iterator_create(map)

/**
 * Create an iterator for the elements of the map.
 * It is the iterator of the underlying hash map.
 */
#define MAP_ITERATOR_INIT(map_it, map)	dynamic_hash_map_iterator_init(map_it, map)
 ...
 */
#define MAP_ITERATOR_NEXT(map_it)		dynamic_hash_map_iterator_next(map_it)

/**
 * Free the space for the given map iterator.
 */
#define MAP_ITERATOR_FREE(map_it)		dynamic_hash_map_iterator_free(map_it)
